        ...
        <div class="col">
            <ul class="nav nav-pills">
                <li class="nav-item col btn">
                    <a href="{{ url('fatty/main/admin/today_rider_billing/data_list/'.$rider_id) }}" class="nav-link" style="background-color:#FFFFFF;width: 100%;border-color:#00dfc2;border-style:solid;border-width:2px;color: #1c1a1a;font-size:15px;font-weight:510;border-radius:5px;">Offered List</a>
                </li>
                <li class="nav-item col btn">
...
<div class="container mt-4" style="padding-left: 20px;padding-right:20px;" id="tabcontent">
    <div class="row tab-pane fade show active" id="history" role="tabpanel" aria-labelledby="history-tab">
        <div class="col">
            <form action="{{ route('today_rider_billing_history.search',$rider_id) }}">
        ...
        @if($check)
            @foreach ($rider_payment as $value)
                <a href="{{ url('fatty/main/admin/today_rider_billing/data_history/detail/'.$value->rider_today_payment_id) }}" class="col mt-3">
